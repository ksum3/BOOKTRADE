		...
		<%
			...
			try {
				...
				ResultSet rs = stmt.executeQuery(
						"select * from book_info, book_regist_info where book_info.book_info_Id=book_regist_info.book_fore and book_regist_info.book_member_Id="+person_id);

				out.println("<table class='table table-hover'>");
				out.println("<caption>책 리스트</caption>");
				out.println("<thead><tr><th>책번호</th><th>등록번호</th><th>책이름</th><th>저자</th><th>출판사</th><th>상태</th><th>삭제하기</th><th>확인</th></tr></thead>");
				...
				while (rs.next()) {
					...
					out.println("<td width=50><a href='booksearch-edit.jsp?book_regist_Id=" + book_regist_Id + "'>"+ book_title + "</a></td>");
					out.println("<td width=100>" + book_author + "</td>");
					out.println("<td width=100>" + book_publisher + "</td>");
					
					if(lend_possible==0){
						out.println("<td width=80>대여가능</td>");
					}else if(lend_possible==1){
						out.println("<td width=80>대여대기</td>");
					}else{
						out.println("<td width=80>대여중</td>");
					}
					out.println("<td width=100><a href='deletebook.jsp?book_regist_Id=" + book_regist_Id
							+ "'>삭제</a></td>");
					if(lend_possible==2){
						out.println("<td width=100><a href='returncheck.jsp?book_regist_Id=" + book_regist_Id
								+ "'>반납완료</a></td>");
					}else{
