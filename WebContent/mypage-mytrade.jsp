	...
	<div class="container">
		...
		<div class="collapse navbar-collapse justify-content-end"
			...
			<ul class="navbar-nav">
				...
				<li class="nav-item active"><a class="nav-link" href="mypage-mytrade.jsp">내 거래 내역<span class="sr-only">(current)</span></a></li>
				<li class="nav-item"><a class="nav-link"
	...
	<div class="card text-center">
		...
		<%
			try{
				...
				ResultSet rs = stmt.executeQuery("select * from trade_info where buyer_id="+person_id);
				
				//책이름 저자 판매자id 대여시작일 대여종료일 반납여부 보증금반환여부를 보여주는 표 필요.
				out.println("<table class='table table-hover'>");
				out.println("<thead><tr><th>도서ID</th><th>대여시작일</th><th>대여종료일</th><th>상태</th></tr></thead>");
				out.println("<tbody>");
				while (rs.next()) {
					...
					int deposit_check = rs.getInt("deposit_check");
					int return_check = rs.getInt("return_check");
					...
					if(return_check==1){
						out.println("<td width=150> 대여 신청 대기 </td>");
					}else if(return_check==2){
						out.println("<td width=150>대여중</td>");
					}else{
						if(deposit_check==2){
							out.println("<td width=150>반납완료</td>");
						}else if(deposit_check==3){
							out.println("<td width=150>처리완료</td>");
						}
					}
					out.println("</tr>");
				}
